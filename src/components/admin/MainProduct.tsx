...
const MainProduct = () => {
    ...
    const onSubmit = (values: ProductFormValues) => {
        ...
        if (dataToSend.images && Array.isArray(dataToSend.images) && dataToSend.images.length > 0) {
            ...
            imageFiles.forEach((file, index) => {
                if (file instanceof File) {
                    ...
                    console.log(`Appending image ${index + 1}/${imageFiles.length}: ${file.name} (${Math.round(file.size/1024)}KB)`);
                }
            });
        } else {
        ...
        for (const pair of Array.from(formData.entries())) {
            if (pair[0] === 'images') {
                ...
                if (pair[1] instanceof File) {
                    console.log(`Image ${imageCount}: ${pair[1].name} (${Math.round((pair[1] as File).size/1024)}KB)`);
                } else {
                                    ...
                                    onDrop={(e) => {
                                        ...
                                        if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                                            ...
                                            if (files.length > 0) {
                                                ...
                                                const newUrls = updatedFiles.map(file => URL.createObjectURL(file));
                                                setPreviewUrls(newUrls);
                                                
                                                // Update form value with all files
                                                onChange(updatedFiles);
                                                
                                                console.log(`Added ${files.length} images, total: ${updatedFiles.length}`);
                                            }
                                        }
                                    }}
                                >
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (files && files.length > 0) {
                                                    ...
                                                    console.log(`Added ${newFiles.length} images, total: ${updatedFiles.length}`);
