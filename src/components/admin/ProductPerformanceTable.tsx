...
const ProductPerformanceTable = () => {
  const [timeFilter, setTimeFilter] = useState<
    "last7days" | "last30days" | "lastyear" | "all"
  >("last7days");
  ...
  // console.log()
  const totalItems = productAnalytics?.data?.pagination?.total || 0;
      ...
      <Card>
        <CardContent className="overflow-x-auto p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="min-w-[220px] text-left px-6 py-4 font-medium text-muted-foreground">
                  Product
                </TableHead>
                        ...
                        <div
                          className={`h-2 rounded-full transition-all ${
                            product.retentionRate >= 50
                              ? "bg-green-500"
                              : product.retentionRate >= 20
                              ? "bg-orange-400"
                              : "bg-red-500"
                          }`}
                          style={{
                            width: `${Math.min(product.retentionRate, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
