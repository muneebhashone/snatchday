...
export const CompetitionForm: React.FC<CompetitionFormProps> = ({ id, mode = 'create', onSuccess }) => {
...
const [open, setOpen] = useState(false);
  ...
  useEffect(() => {
    if (data?.pages) {
      ...
      const uniqueProducts = Array.from(new Map(allProducts.map((p: any) => [p._id, p])).values());
      setProductOptions(uniqueProducts.map((p: any) => ({ value: p._id, label: p.name })));
  ...
  useEffect(() => {
    if (
      id && competition && competition.product &&
  ...
  useEffect(() => {
    if (id && competition) {
      form.reset({
        ...
        month: competitionData.month ? (new Date(competitionData.month).getMonth() + 1).toString() : '',
  ...
  if (
    id && competition && selectedProductId &&
  ...
  const onSubmit = async (values: CompetitionFormValues) => {
    try {
      ...
      toast.error(error?.response?.data?.message || 'Failed to submit competition');
    }
  };
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        />
        <Button type="submit" className="w-full">{mode === 'edit' ? 'Update Competition' : 'Create Competition'}</Button>
      </form>
    </Form>
  );
}; 